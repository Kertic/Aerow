using System;
using Code.Player.States.SubStates.Airborne;
using Code.Player.States.SubStates.Grounded;
using Code.Player.States.SuperStates;
using UnityEngine;
...
namespace Code.Player.States
{
    ...
    public class PlayerStateMachine : MonoBehaviour
    {
        ...
        public Jumping _Jumping { get; private set; }
        public Falling _Falling { get; private set; }
        ...
        private void Awake()
        {
            ...
            _Jumping = new Jumping(_PlayerData, _PlayerPhysics, this);
            _currentState = _Falling = new Falling(_PlayerData, _PlayerPhysics, this);
            _controls = new Controls();
            _controls.Gameplay.Movement.performed += OnMovementInput;
            _controls.Gameplay.Jump.performed += PressJumpInput;
            _controls.Gameplay.PrimaryFire.performed += (InputAction.CallbackContext context) => { PressFireInput(true, context); };
            _controls.Gameplay.SecondaryFire.performed += (InputAction.CallbackContext context) => { PressFireInput(false, context); };

            _controls.Gameplay.Movement.canceled += OnMovementInputEnd;
            _controls.Gameplay.Jump.canceled += ReleaseJumpInput;
        ...
        private void PressFireInput(bool isPrimary, InputAction.CallbackContext context)
        {
            if (isPrimary)
            {
        ...
        {
            _currentState.OnReceiveButtonInput(PlayerState.InputButton.JUMP);
        }

        private void ReleaseJumpInput(InputAction.CallbackContext context)
        {
            _currentState.OnReleaseButtonInput(PlayerState.InputButton.JUMP);
        }
